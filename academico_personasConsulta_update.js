...
function pideRellenarMaterias() {
	...
	$.ajax({
		...
		type : 'POST',
		async : true,
		data : 'rellenaMateriasConsulta=rellenaMateriasConsulta&matricula=' + matricula,
...
function generaHTMLpaso2(matricula, nombre, check, check_nombres) {
	...
	$("#btn_actulizar").click(pideActualizarConsulta);
...
function pideActualizarConsulta() {
	...
	$.ajax({
		...
		type : 'POST',
		async : true,
		data : 'academicoConsultaUpdate=academicoConsultaUpdate&matricula=' + matricula + "&datos=" + json_datos,
		success : actualizaConsulta,
	});

}

function actualizaConsulta(datos) {
